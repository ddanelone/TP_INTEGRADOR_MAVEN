...
public class GrafoControlador implements ActionListener {
	...
	private Graph createGraphWithPageRank() {
		...
		// Obtener la lista de vértices ordenados por PageRank con factore de amortiguación 0.85 y 100 iteraciones
	...
	private void drawArrow(Graphics g, int x1, int y1, int x2, int y2) {
		...
		g2d.fillPolygon(
				new int[] { x2, (int) (x2 - arrowSize * Math.cos(angle - Math.PI / 6)),
						(int) (x2 - arrowSize * Math.cos(angle + Math.PI / 6)) },
				new int[] { y2, (int) (y2 - arrowSize * Math.sin(angle - Math.PI / 6)),
